...
function CalendarStatusFilters({ counts, onClear, onToggle, selected }: Readonly<CalendarStatusFiltersProps>) {
  return (
    <div className={`shrink-0 border-b bg-ui-surface-muted/90 px-2 py-1.5 backdrop-blur-sm ${themeClass.border.default} md:px-3`}>
      <div className="flex items-center gap-2 overflow-x-auto px-1 py-1.5 custom-scrollbar" aria-label="Filtros rápidos por estado">
        ...
        {selected.length > 0 ? (
          <button
            ...
            onClick={onClear}
            className={`inline-flex size-7 shrink-0 items-center justify-center rounded-full shadow-sm transition-colors ${themeClass.surface.default} ${themeClass.interactive.ghost} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-400/60`}
          >
